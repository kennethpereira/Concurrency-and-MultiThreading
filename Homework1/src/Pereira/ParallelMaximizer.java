...
public class ParallelMaximizer {
	...
	public static void main(String[] args) {
		...
		// run the maximizer
		try {
			System.out.println("Partial Max is:"+maximizer.max(list,numThreads));
	...
	public int max(LinkedList<Integer> list, int numThreads) throws InterruptedException {
		...
		for(ParallelMaximizerWorker parallerWorker:workers){
			max = Math.max(max, parallerWorker.getPartialMax());
		}
